...
def find_point(data):
	...
	#if msg:
	#    print "Received a MAVLINK message"
	#    print msg.get_type()
	#    if msg.get_type() == "OPTICAL_FLOW":
	#        print "X velocity is %f"%msg.flow_comp_m_x
	#        print "Y velocity is %f"%msg.flow_comp_m_y
	#    if msg.get_type() == "LOCAL_POSITION_NED":
	#	print "Local x position is %f"%msg.x
		
	
        if abs(pointx) > 0.1: #0.15->0.1, 3/31/2014
            if pointx > 0:
		print "         "
		print "          \  "
		print "___________\ "
...
#		print "         "
	...
	#   master.mav.set_quad_swarm_roll_pitch_yaw_thrust_send(1, 2, 0, 10000,0, throttlecmd)
            master.mav.set_quad_swarm_roll_pitch_yaw_thrust_send(1, 2, pointx/abs(pointx)*17500, 0, 0, throttlecmd)  
	    rospy.sleep(0.35)  
	    master.mav.set_quad_swarm_roll_pitch_yaw_thrust_send(1, 2, 0, 0, 0, throttlecmd)
	    
        else:
            if pointz > 0.5: #and abs(pointx) < 0.05:
...
#		print "|"
#		print "         "
                master.mav.set_quad_swarm_roll_pitch_yaw_thrust_send(1, 2, 0, -17500, 0, throttlecmd)
		rospy.sleep(0.35)                       # THIS WILL DETERMINE HOW LONG A GIVEN COMMAND SHOULD LAST 
		master.mav.set_quad_swarm_roll_pitch_yaw_thrust_send(1, 2, 0, 0, 0, throttlecmd)  #THIS WILL STOP THE QUAD
